...
__all__ = ["UnetMonaiChannelDO"]


@export("monai.networks.nets")
@alias("Unet")
class UnetMonaiChannelDO(nn.Module):
    """
    ...
    def __init__(
        ...
        self.up2 = nn.Sequential(
                Convolution(3, self.channels[1]+self.channels[1], self.channels[0], strides=self.strides[1],
                kernel_size=self.up_kernel_size, act=self.act, norm=self.norm, dropout=self.dropout, 
                bias=self.bias, conv_only=False, is_transposed=True, adn_ordering=self.adn_ordering),
        ...
        self.up1 = nn.Sequential(
                Convolution(3, self.channels[0]+self.channels[0], self.out_channels, strides=self.strides[0],
                kernel_size=self.up_kernel_size, act=self.act, norm=self.norm, dropout=self.dropout, 
                bias=self.bias, conv_only=False, is_transposed=True, adn_ordering=self.adn_ordering),
    ...
    def channel_dropout(self, x, is_WDO = False):
        ...
        if not is_WDO:
            # torch.randperm is used for random shuffle of the channel
            # idx = torch.randperm(x.shape[1])
            # x = x[:, idx, :, :, :]

            # Create a random tensor of the same shape as the channel dimension
            channel_dice = torch.rand(x.size(1), device=x.device)
            
            # Create a mask for the channels that need to be dropped
            dropout_mask = (channel_dice < self.dropout_rate).float().view(1, -1, 1, 1, 1)
            
            # Apply the mask to the input tensor x
            x = x * dropout_mask

        else:
            # Compute the mean value µ for each channel across the batch, height, width, and depth
            channel_means = x.mean(dim=(0, 2, 3, 4), keepdim=True)

            # Generate a random number r for each channel
            random_numbers = torch.rand_like(channel_means)

            # Calculate the keys for each channel as r_i^(1/µ)
            keys = random_numbers ** (1 / channel_means)
